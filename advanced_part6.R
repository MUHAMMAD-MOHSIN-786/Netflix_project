...
# let's make function for a single title
analyseShow <- function(title){
    showData <- netflixViews_Cleaned %>%
        filter(Title == title)
    
    # let's give some new cols
    times <- showData %>%
        select(Start.Time, Duration) %>% #To simplify the table temporarily 
        mutate(DayofWeek = wday(Start.Time, label = TRUE, abbr = FALSE),
            Hour = hour(Start.Time),
            TimeofDay = case_when(Hour <= 12 ~ "Morning",
                Hour >= 16 ~ "Evening",
...
analyseShow("The Big Flower Fight")
analyseShow("Call My Agent!")
